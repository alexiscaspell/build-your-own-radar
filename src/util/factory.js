...
const GoogleSheetInput = function() {
    ...
    self.build = function() {
        ...
        if (domainName && queryParams.sheetId.endsWith('csv')) {
            ...
            sheet.init().build()
        } else if (queryParams.sheetId && (queryParams.sheetId.startsWith("Sheet_") || domainName && domainName.endsWith('google.com'))) {
            const loadedSheetId = queryParams.sheetId.startsWith("Sheet_") ? queryParams.sheetId.split("Sheet_")[1] : queryParams.sheetId.split("/d/")[1].split("/")[0]
...
function plotForm(content) {
    ...
    form.append('input')
        .attr('type', 'text')
        .attr('name', 'sheetId')
        .attr('placeholder', 'e.g. Sheet_<sheetid> or https://docs.google.com/spreadsheets/d/<sheetid> or hosted CSV file')
        .attr('required', '')
